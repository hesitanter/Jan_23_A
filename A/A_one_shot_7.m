...
function A_one_shot_7(input, thre)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
draw_plot(Y, test_label, input, thre);
...
function result_analysis(result, input, truth, truth_time)
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % write down the time step detected
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    sz = size(result);
    if length(result) > 6
        ...
        j = 1;
        count = 1;
        time_slip = zeros(length(result), 1);
        for i = 1:sz        
...
function average_exercise_bio_signal(pre_data)
...
legend
hold off
saveas(h, sprintf('A_glucose_%d.fig', input));
end
